...
- (VoronoiResult *)computeWithSites:(NSMutableArray *)cellTowerList andBoundingBox:(CGRect)bbox
{
    ...
    double xsitex = FLT_MIN; // To avoid duplicate sites
    double xsitey = FLT_MIN;
...
- (void)addBeachsection:(Site *)site
{
    ...
    if (lArc != rArc) {
        ...
        Vertex *vertex = [[Vertex alloc] initWithCoord:CGPointMake((cy*hb - by*hc)/d+ax,(bx*hc - cx*hb)/d+ay)];
...
- (void)removeBeachsection:(Beachsection *)bs
{
    ...
    Vertex *vertex = [[Vertex alloc] initWithCoord:CGPointMake(x, y)];
    ...
    while ([lArc circleEvent] && fabs(x - [[lArc circleEvent] x]) < VORONOI_EPSILON && fabs(y - [[lArc circleEvent] ycenter]) < VORONOI_EPSILON) {
    ...
    while ([rArc circleEvent] && fabs(x - [[rArc circleEvent] x]) < VORONOI_EPSILON && fabs(y - [[rArc circleEvent] ycenter]) < VORONOI_EPSILON) {
        next = [rArc rbNext];
...
- (double)rightBreakPointWithArc:(Beachsection *)arc 
                   andDirectrix:(double)directrix
{
...
- (double)leftBreakPointWithArc:(Beachsection *)arc 
                  andDirectrix:(double)directrix
{
...
- (BOOL)connectEdge:(Edge *)edge withBoundingBox:(CGRect)bbox
{
    ...
    Vertex *va = [edge va];
    double xl = bbox.origin.x;
    double xr = bbox.origin.x + bbox.size.width;
    double yt = bbox.origin.y;
    double yb = bbox.origin.y + bbox.size.height;
    ...
    if (ry != ly) {
        ...
        if (fm < -1 || fm > 1) {
            ...
            if (lx > rx) {
                if (!va) {
                    va = [[Vertex alloc] initWithCoord:CGPointMake((yt-fb)/fm, yt)];
                } else if ([va y] >= yb) {
                    return NO;
                }
                vb = [[Vertex alloc] initWithCoord:CGPointMake((yb-fb)/fm, yb)];
            } else {
                // upward
                if (!va) {
                    va = [[Vertex alloc] initWithCoord:CGPointMake((yb-fb)/fm, yb)];
                } else if ([va y] < yt) {
                    return NO;
                }
                vb = [[Vertex alloc] initWithCoord:CGPointMake((yt-fb)/fm, yt)];
            }
        } else {
            ...
            if (ly < ry) {
                if (!va) {
                    va = [[Vertex alloc] initWithCoord:CGPointMake(xl, fm*xl+fb)];
                } else if ([va x] >= xr) {
                    return NO;
                }
                vb = [[Vertex alloc] initWithCoord:CGPointMake(xr, fm*xr+fb)];
            } else {
                // leftward
                if (!va) {
                    va = [[Vertex alloc] initWithCoord:CGPointMake(xr, fm*xr+fb)];
                } else if ([va x] < xl) {
                    return NO;
                }
                vb = [[Vertex alloc] initWithCoord:CGPointMake(xl, fm*xl+fb)];
            }
        }
        ...
        if (lx > rx) {
            if (!va) {
                va = [[Vertex alloc] initWithCoord:CGPointMake(fx, yt)];
            } else if ([va y] >= yb) {
                return NO;
            }
            vb = [[Vertex alloc] initWithCoord:CGPointMake(fx, yb)];
        } else {
            // upward
            if (!va) {
                va = [[Vertex alloc] initWithCoord:CGPointMake(fx, yb)];
            } else if ([va y] < yt) {
                return NO;
            }
            vb = [[Vertex alloc] initWithCoord:CGPointMake(fx, yt)];
        }
    }
...
- (BOOL)clipEdge:(Edge *)edge withBoundingBox:(CGRect)bbox
{
    ...
    if (t0 > 0) {
        [edge setVa:[[Vertex alloc] initWithCoord:CGPointMake(ax+t0*dx, ay+t0*dy)]];
    }
    ...
    if (t1 < 1) {
        [edge setVb:[[Vertex alloc] initWithCoord:CGPointMake(ax+t1*dx, ay+t1*dy)]];
    }
...
- (void)clipEdges:(CGRect)bbox
{
    ...
    while (iEdge--) {
        ...
        if (![self connectEdge:edge withBoundingBox:bbox] 
            ...
            || (fabs([[edge va] x] - [[edge vb] x]) < VORONOI_EPSILON && fabs([[edge va] y] - [[edge vb] y]) < VORONOI_EPSILON)) {
            [edge setVb:nil];
            [edge setVa:nil];
...
- (void)closeCells:(CGRect)bbox
{
    ...
    while (iCell--) {
        ...
        while (iLeft < nHalfedges) {
            ...
            if (fabs(endPointX - startPointX) >= VORONOI_EPSILON || fabs(endPointY - startPointY) >= VORONOI_EPSILON) {
                ...
                if ([Voronoi equalWithEpsilonA:[endpoint x] andB:xl] && [Voronoi lessThanWithEpsilonA:[endpoint y] andB:yb]) {
                    ...
                    double tempY = [Voronoi equalWithEpsilonA:[startpoint x] andB:xl] ? [startpoint y] : yb;
                    vb = [[Vertex alloc] initWithCoord:CGPointMake(xl, tempY)];
                    ...
                    double tempX = [Voronoi equalWithEpsilonA:[startpoint y] andB:yb] ? [startpoint x] : xr;
                    vb = [[Vertex alloc] initWithCoord:CGPointMake(tempX, yb)];
                    ...
                    double tempY = [Voronoi equalWithEpsilonA:[startpoint x] andB:xr] ? [startpoint y] : yt;
                    vb = [[Vertex alloc] initWithCoord:CGPointMake(xr, tempY)];
                    
                } else if ([Voronoi equalWithEpsilonA:[endpoint y] andB:yt] && [Voronoi greaterThanWithEpsilonA:[endpoint x] andB:xl]) {
                    
                    // walk leftward along top side
                    double tempX = [Voronoi equalWithEpsilonA:[startpoint y] andB:yt] ? [startpoint x] : xl;
                    vb = [[Vertex alloc] initWithCoord:CGPointMake(tempX, yt)];
...
{
    return fabs(a - b) < VORONOI_EPSILON;
}
